...
function renderSkillOptions(students: Student[]): void {
	...
	const uniqueSkills = [...new Set(students.flatMap((student) => student.skills))].sort((a, b) =>
		a.localeCompare(b)
	);
...
function renderResultsCount(visible: number, total: number): void {
	const results = document.getElementById("results-count") as HTMLOutputElement | null;
	if (results === null) {
		return;
	}
...
function renderBioCount(length: number, maxLength: number): void {
	const bioCount = document.getElementById("bio-count") as HTMLOutputElement | null;
	if (bioCount === null) {
		return;
	}
